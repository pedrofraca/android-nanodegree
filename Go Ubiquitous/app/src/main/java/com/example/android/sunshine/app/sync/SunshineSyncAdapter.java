...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
            ...
            if ( cVVector.size() > 0 ) {
                ...
                Cursor cursor = getContext().getContentResolver().query(WeatherContract.WeatherEntry.CONTENT_URI,
                        ...
                        WeatherContract.WeatherEntry.COLUMN_DATE + " = ? AND " +
                                WeatherContract.WeatherEntry.COLUMN_WEATHER_ID + " = ? AND " +
                                WeatherContract.WeatherEntry.COLUMN_MAX_TEMP + " = ? AND " +
                                WeatherContract.WeatherEntry.COLUMN_MIN_TEMP + " = ?",
                        new String[]{Long.toString(dayTime.setJulianDay(julianStartDay)),
                                Integer.toString(cvArray[0].getAsInteger(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID)),
                                Double.toString(cvArray[0].getAsDouble(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP)),
                                Double.toString(cvArray[0].getAsDouble(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP)),
                        },
                        null);
                //We want to check if we have data changes to notify to the watch
                if(cursor != null && !cursor.moveToFirst()){
                    //Nothing to compare, so notifying to the wear device
                    notifyWeatherToWear(cvArray[0].getAsDouble(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP),
    ...
    private static Asset createAssetFromBitmap(Bitmap bitmap) {
        ...
        newBitmap.compress(Bitmap.CompressFormat.PNG, 100, byteStream);
        return Asset.createFromBytes(byteStream.toByteArray());
    }

    private void notifyWeatherToWear(Double min, Double max, Integer weatherId) {
        Log.d(LOG_TAG, "Notifying to the android wear. Max" + max +" " + min + " " +weatherId);

        PutDataMapRequest putDataMapReq = PutDataMapRequest.create("/sunshine");
        putDataMapReq.getDataMap().putDouble(MIN_TEMP, min);
        putDataMapReq.getDataMap().putDouble(MAX_TEMP, max);
        Bitmap icon = BitmapFactory.decodeResource(getContext().getResources(),
                Utility.getArtResourceForWeatherCondition(weatherId));
        Asset iconAsset = createAssetFromBitmap(icon);
        putDataMapReq.getDataMap().putLong(LOG_TAG, System.currentTimeMillis());
        putDataMapReq.getDataMap().putAsset(WEATHER_ICON,iconAsset);
